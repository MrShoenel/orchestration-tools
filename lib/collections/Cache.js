const { Dictionary, DictionaryMapBased } = require('./Dictionary')
  ...
  constructor(evictPolicy = EvictionPolicy.None, capacity = Number.MAX_SAFE_INTEGER, eqComparer = EqualityComparer.default) {
	...
	*_evictNext() {
	...
	set(key, val) {
		...
		const wrap = new CacheItem(key, val, CacheMapBased._counter);
	...
	get(key) {
		...
		return this.__unwrapValue(wrap);
	};
...
class CacheItemWithLoad extends CacheItem {
	constructor(key, item, load, order) {
		super(key, item, order);
