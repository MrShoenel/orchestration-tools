...
class ProcessWrapper extends EventEmitter {
   ...
   * Observable created will just observe any emits. Thus it never errors and never
   * drains. This Observable can always be created, regardless of whether the Process
   * is running or not. This Observable thus is only a convenient alternative to
   * adding listeners to this instance.
   * 
   * @returns {Observable.<ProcessOutput>}
   */
  get observable() {
    return Observable.fromEvent(this, symbolProcessOutput);
  };
  ...
  spawnObservable() {
    return new Observable(async subs => {
      const subscription = Observable.fromEvent(this, symbolProcessOutput)
        .subscribe(next => {
          subs.next(next);
        });
  ...
  /**
   * Launces the process using spawn().
   * 
      ...
      return procExit => {
        if (!calledAlready) {
          ...
          if (procExit instanceof ProcessErrorResult
            ...
            deferred.reject(procExit);
          } else {
            deferred.resolve(procExit);
          }
        }
      };
    })();
