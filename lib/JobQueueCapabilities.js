...
class JobWithCost extends Job {
  ...
  /**
   * Returns a new JobWithCost by using the given Job's promiseProducer and
   * the given cost. Also uses its Progress, if configured.
   * 
   * @param {Job} job 
   * @param {number} cost 
   * @returns {JobWithCost}
   */
  static fromJob(job, cost) {
    const jobWithCost = new JobWithCost(job._promiseProducer, cost);
    jobWithCost.progress = job.progress;
    return jobWithCost;
  };

  /**
   * Convert this JobWithCost to an ordinary Job without cost.
   * 
   * @returns {Job}
   */
  toJob() {
    const job = new Job(this._promiseProducer);
    job._progressInstance = this._progressInstance;
    return job;
  };
};
