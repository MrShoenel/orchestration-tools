...
class Job extends EventEmitter {
  /**
   * @template T, TProgress
   * @param {() => Promise.<T>} promiseProducer
   * @param {Progress.<TProgress>} progress should be supplied if this
   * job ought to support progress.
   */
  constructor(promiseProducer, progress = void 0) {
    super();
    if (!(promiseProducer instanceof Function)) {
      throw new Error(`The given promiseProducer is not a function.`);
    }
    if (progress !== void 0 && !(progress instanceof Progress)) {
      throw new Error('If given, the progress must be an instance of Progress.');
    }
    ...
    /** @type {TProgress} */
    this._lastProgress = void 0;

    if (progress instanceof Progress) {
      this.supportsProgress = true;
      progress.observable.subscribe(progress => {
  ...
  /**
   * Returns the last progress reported.
   * 
   * @returns {TProgress}
   */
  get progress() {
